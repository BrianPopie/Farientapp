...
function RoleAwareChat() {
  ...
  const ask = (question: string) => {
    ...
    if (!trimmed) return;
    send(userPrompt(band, role, company, trimmed));
    setInput("");
  };

  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[290px_1fr_360px]">
      <aside className="space-y-4">
        <Card className="space-y-3 p-4">
          ...
          <Select
            value={band}
            onValueChange={(value) => {
              const nextBand = value as BandKey;
              setBand(nextBand);
              const fallbackBand = ROLE_BANDS.find((b) => b.key === nextBand) ?? ROLE_BANDS[0];
      ...
      <section className="flex flex-col">
        ...
        <div className="mt-3 flex gap-2">
          ...
          />
          <Button disabled={streaming || !input.trim()} onClick={() => ask(input)}>
      ...
      <aside className="space-y-4">
        <Card className="p-4">
          ...
          <ul className="mt-2 space-y-2 text-xs text-text-muted">
            <li>
              <strong>cit_aur_sct_pg32</strong> - DEF 14A, Summary Comp Table (p.32 lines 10-22)
            </li>
            <li>
              <strong>cit_aur_pvp_pg52</strong> - DEF 14A, Pay vs Performance (p.52 lines 1-25)
            </li>
            <li>
              <strong>cit_bor_retention</strong> - 8-K retention award (p.3 lines 15-24)
            </li>
            <li>
              <strong>iss_2026_policy</strong> - ISS 2026 Policy section 2.1 P4P overlay
            </li>
          </ul>
        </Card>
        <Card className="space-y-3 p-4">
          ...
          {peers.length === 0 ? (
            ...
            <ul className="space-y-2 text-xs">
              {peers.map((peer) => (
                <li key={peer.ticker} className="rounded-xl border border-border/60 bg-surface/50 p-2">
        ...
        <Card className="space-y-3 p-4">
          ...
          <div className="grid gap-2">
            <Button size="sm" variant="secondary" className="justify-start" disabled>
