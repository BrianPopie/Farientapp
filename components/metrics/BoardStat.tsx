...
export function BoardStat({ label, value, valueClassName }: BoardStatProps) {
  return (
    <div className="rounded-2xl border border-border bg-white/80 px-4 py-3 text-sm text-text shadow-inner dark:border-white/10 dark:bg-white/5 dark:text-white/80">
      ...
      <p className={cn("mt-1 text-lg font-semibold text-text dark:text-white", valueClassName)}>{value}</p>
    </div>
  );
}
