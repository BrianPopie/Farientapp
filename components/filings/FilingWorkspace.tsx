...
const UploadDropzone = dynamic(() => import("@/components/UploadDropzone").then((mod) => mod.UploadDropzone), {
  ssr: false,
  loading: () => (
    <div className="flex min-h-[320px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-border/60 bg-muted/50 text-xs text-muted-foreground">
      Preparing uploader…
    </div>
  )
});
...
type FilingWorkspaceProps = { subtitle: string };

export function FilingWorkspace({ subtitle }: FilingWorkspaceProps) {
    ...
    <div className="space-y-10">
      ...
      <section className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {uploads.length === 0 ? (
            <p className="text-xs text-muted-foreground">Drop a PDF dummy file to see it appear here instantly.</p>
          ) : null}
      ...
      <section className="rounded-3xl border border-border bg-card p-6 shadow-elev-0">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <DataTable<(typeof summaryCompTable)[number]>
              columns={[
                { key: "exec", header: "Executive" },
                {
                  ...
                  align: "right",
                  render: (row) =>
                    currency.format((row as (typeof summaryCompTable)[number]).salary)
                },
                {
                  key: "bonus",
                  header: "Bonus",
                  align: "right",
                  render: (row) =>
                    currency.format((row as (typeof summaryCompTable)[number]).bonus)
                },
                {
                  key: "equity",
                  header: "Equity",
                  align: "right",
                  render: (row) =>
                    currency.format((row as (typeof summaryCompTable)[number]).equity)
                },
                {
                  key: "mix",
                  header: "Mix",
                  render: (row) => <Badge>{formatMixPill(row as typeof summaryCompTable[number])}</Badge>
                }
              ]}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Grant plan tie-outs</CardTitle>
          </CardHeader>
          <CardContent>
            <DataTable<(typeof grantsTable)[number]>
              columns={[
        ...
        <Card>
          ...
          <CardContent>
            <DataTable<(typeof pvpTable)[number]>
              columns={[
                { key: "year", header: "Year" },
                { key: "ceoPay", header: "CEO pay" },
                { key: "compPeerPct", header: "Comp peer pct" },
                { key: "tsr", header: "TSR" }
              ]}
              data={pvpTable}
            />
      ...
      <section className="rounded-3xl border border-border bg-card p-6 shadow-elev-0">
        <div className="flex flex-wrap gap-4">
          <Card className="flex-1 border border-border/80 bg-card/50">
            <CardHeader>
              <CardTitle>Entity normalization</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Tickers</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {entityNormalization.tickers.map((ticker) => (
                    <Badge key={ticker.label} variant="outline">
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Executives</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {entityNormalization.executives.map((exec) => (
                    <Badge key={exec.label} variant="outline">
                      {exec.label} · {exec.status}
                    </Badge>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">Grants</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  {entityNormalization.grants.map((grant) => (
                    <Badge key={grant.label} variant="outline">
                      {grant.label} · {grant.status}
                    </Badge>
                  ))}
                </div>
              </div>
          ...
          <Card className="flex-1 border border-border/80 bg-card/50">
            ...
            <CardContent className="space-y-3">
              <div className="flex flex-wrap gap-2">
                {(citations as Citation[]).slice(0, 6).map((citation) => (
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <Card className="border border-border/80 bg-card">
          <CardHeader>
            <CardTitle>Agent provenance</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 text-sm text-muted-foreground">
            <p>
              Gather step completed in 45 seconds with 98.4% OCR confidence. Narrative extraction is currently running and queued filings will be
              indexed once the executive tables are validated.
            </p>
            <ul className="list-disc space-y-2 pl-5">
              <li>28 issuers queued; 1 requires OCR fallback.</li>
              <li>Table extraction flagged 2 variance checks for review.</li>
              <li>Citation refresh scheduled for 18:00 UTC.</li>
            </ul>
          </CardContent>
        </Card>
        <AiInsightPanel
